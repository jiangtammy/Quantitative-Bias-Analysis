#QBA random forests - non-differential misclassification of predictor for simulated data set #1
#March 21, 2019
library(foreign)
...
################################# True data set #1 #################################
################################# Data set #1: Non-differential misclassification of predictor x5 #################################
...
rf.dat1.nondiff.pred <- randomForest(y ~., data=dat1.nondiff.pred2, importance=T)
...
dat1.nondiff.misc.pred.func.result <- t(sapply(1:1000, dat1.nondiff.misc.pred.func, se=myse,sp=mysp))

write.csv(dat1.nondiff.misc.pred.func.result, col.names=F,
          paste0("results/dat1_nondiff_misc_predictor_myse_", myse, "_mysp_", mysp, "_", 
