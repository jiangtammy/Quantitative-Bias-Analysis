...
#Bias adjustment for differential misclassification of outcome 
...
dat1.diff.cor.outcome.func <- function(i, se1, sp1, se0, sp0){
  ...
  repeat {
    ...
    dat1.diff.outcome1 <- dat1
    
    dat1.diff.outcome1$y.new <- NA
    dat1.diff.outcome1$y.new[dat1.diff.outcome1$y==1 & dat1.diff.outcome1$x5==1] <- rbinom(length(which(dat1.diff.outcome1$y==1 & dat1.diff.outcome1$x5==1)), 1, se1)
    dat1.diff.outcome1$y.new[dat1.diff.outcome1$y==0 & dat1.diff.outcome1$x5==1] <- rbinom(length(which(dat1.diff.outcome1$y==0 & dat1.diff.outcome1$x5==1)), 1, 1-sp1)
    dat1.diff.outcome1$y.new[dat1.diff.outcome1$y==1 & dat1.diff.outcome1$x5==0] <- rbinom(length(which(dat1.diff.outcome1$y==1 & dat1.diff.outcome1$x5==0)), 1, se0)
    dat1.diff.outcome1$y.new[dat1.diff.outcome1$y==0 & dat1.diff.outcome1$x5==0] <- rbinom(length(which(dat1.diff.outcome1$y==0 & dat1.diff.outcome1$x5==0)), 1, 1-sp0)
    
    
    #Subset data for misclassified variables and the other variables of interest
    vars <- c("x1", "x2", "x3", "x4", "x5", "y.new")
    dat.diff.outcome <- dat1.diff.outcome1[vars] 
    
    names(dat.diff.outcome) <- c("x1", "x2", "x3", "x4", "x5", "y")
    dat.diff.outcome$y <- as.factor(dat.diff.outcome$y)
    ...
    ############For persons with the predictor (X5=1)############
    dat.se1.sp1 <- dat3[which(dat3$x5==1),] #Replace x5 with name of predictor
    ...
    ############For persons with the predictor (X5=0)############
    dat.se0.sp0 <- dat3[which(dat3$x5==0),] #Change x5 to the predictor
  ...
  rf.dat1.diff.outcome <- randomForest(y.expected ~., data=dat.final, importance=T, sampsize=c(length(which(dat.final$y.expected==1)), length(which(dat.final$y.expected==1)))) 
...
dat1.diff.cor.outcome.func.result <- t(sapply(1:10000, dat1.diff.cor.outcome.func, se1=myse1, sp1=mysp1, se0=myse0, sp0=mysp0)) 
